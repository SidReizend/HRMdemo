...
$sql_select_talents = "SELECT talents.`id`,`average_working_hour_per_day`,"
        . "`average_hour_calculation`,`role`.`role_stream`,rs.monthly_process "
        . "FROM `talents` "
        ...
        if ($roleStream != $managementRolestream) {
            ...
            if (true === $first_time) {
                ...
                if (false == $is_current_day_lop && FALSE === $next_working_day_worked && FALSE == $previous_working_day_lop && FALSE == $lop_deducted) {
                    ...
                    if ($rowTalentAttance['avg_consider'] == 0) {
                        if ($dateCount == date("Y-m-d", strtotime('2017-04-08')) && $talentId == 6) {
                            echo '111<br/>';
                            echo $rowTalentAttance['avg_consider'] . '<br/>';
                            echo $is_current_day_lop . '<br/>';
                            echo $previous_working_day_lop . '<br/>';
                            echo $next_working_day_worked . '<br/>';
                            echo $talentId;
                            echo '<br/>222';
                        }
...
//            }
    ...
    while ($rowTalentLeaveLeft = mysql_fetch_array($resultTalentLeaveLeft)) {
        if ($rowTalentLeaveLeft['leave_type'] == '1') {
            $compOffLeft = $rowTalentLeaveLeft['leave_left'];
        }
        if ($rowTalentLeaveLeft['leave_type'] == '2') {
            $elLeft = $rowTalentLeaveLeft['leave_left'];
        }
    }
    $sqlLeave = "UPDATE `talent_leave_left` SET `leave_left`='0' "
            . "WHERE `talent`='$talentId' ";
    mysql_query($sqlLeave);
    $sqlInsertData = "INSERT INTO `talent_monthly_report_resigned`(`talent_id`, `month`, `year`, "
            ...
            . "`last_day_lop`,`lop_due_to_lop`,`total_extra_hours`,`comp_off_left`,`el_left`,`days_pay`) VALUES "
            ...
            . "'$last_day_lop','$lop_count','$totalExtraHoursInserting','$compOffLeft','$elLeft','$days_pay')";
    //die();
...
function setEl($talentId, $resignDate, $dateOfJoining) {
    ...
    $updateElActual = mysql_query("UPDATE talent_monthly_report_resigned "
            . "SET `el_actual`='$EL',`el_expected`='$elExpected' "
            . "WHERE `talent_id`='$talentId' ") or die(mysql_error());
    return;
}
